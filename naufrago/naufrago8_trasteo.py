...
class Naufrago:
 ...
 def help_about(self, action):
  ...
  about.set_comments("Naufrago! es un lector RSS simple")
 ...
 def create_trayicon(self):
  ...
  update_item = gtk.ImageMenuItem("Actualizar todo")
...
#   treeview.expand_row(path[:i+1], open_all=False)
 ...
 def check_feed_item(self, dentry):
  ...
  if(hasattr(dentry,'id')): id = dentry.id.encode('utf-8')
  else: id = None
 ...
 def toggle_menuitems_sensitiveness(self, enable):
  ...
  item_list = ["/Menubar/ArchivoMenu/Eliminar feed", "/Menubar/ArchivoMenu/Eliminar categoria",
               "/Menubar/ArchivoMenu/Importar feeds", "/Menubar/ArchivoMenu/Salir",
               "/Menubar/EditarMenu/Editar", "/Menubar/EditarMenu/Preferencias",
               "/Menubar/RedMenu/Actualizar", "/Menubar/RedMenu/Actualizar todo",
               "/Toolbar/Actualizar todo", "/Toolbar/Preferencias"]
   ...
   while iter is not None:
    if(model.iter_depth(iter) == 0): # Si es padre
     ...
     for i in range(model.iter_n_children(iter)):
      ...
      cursor.execute('SELECT url FROM feed WHERE id = ?', [id_feed])
      url = cursor.fetchone()[0]
      self.statusbar.set_text('Obteniendo feed ' + nombre_feed + '...')
      ...
      for i in range(0, limit):
       ...
       if(id is not None):
        ...
        print 'No tiene id.'
        # Hacer un md5 del campo description usarlo para comparar.
        id = md5.new(description).hexdigest()
        cursor.execute('SELECT id FROM articulo WHERE entry_unique_id = ? AND id_feed = ?', [id,id_feed])
       ...
       if(unique is None):
        ...
        if((row2 is not None) and (row2[0]>=self.num_entries)):
         ...
         print 'Feed lleno; purgando una entrada para hacer espacio...'
        ...
        if(secs is None):
         print 'Secs es none, se le crea un timestamp...'
        ...
        cursor.execute('INSERT INTO articulo VALUES(null, ?, ?, ?, ?, 0, 0, ?, ?, ?)', [title,description,secs,link,images,id_feed,id])
        self.conn.commit()
