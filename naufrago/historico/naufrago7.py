...
#############################################################################
#
# Requirements apart from GNU/Linux & Python (lib names for Debian GNU/Linux):
# - python-gtk2
# - python-pysqlite2
# - python-feedparser
# - python-webkit
...
#
# Thanks to:
# - #python channel both @ irc.freenode.net & irc.hispano.org
# - The cool svg logo done by: GMcGlinn (http://www.openclipart.org/user-detail/GMcGlinn)
# - The fancy rss svg done by: SRD (http://www.openclipart.org/user-detail/SRD)
#
...
ABOUT_PAGE = '''<html>
               <body>
                <p>
                <font size='4' color='blue'><b>náufrago, ga.</b></font>
                <font color='green'><b>(Del lat. naufrăgus).</b></font></br>
                <font color='blue'><b>1.</b> adj.</font> Que ha padecido naufragio. Apl. a pers.</br>
                <i>(...)</i>
                </p>
                <p>
                <font size='4' color='blue'><b>naufragio</b></font>
                <font color='green'><b>(Del lat. naufragĭum).</b></font></br>
	        <font color='blue'><b>1.</b> m.</font> Pérdida o ruina de la embarcación en el mar o en río o lago navegables.</br>
                <i>(...)</i>
                </p>
                </br><p><b>Náufrago</b> es un lector simple de RSS que permite ver noticias con sus imágenes aún <i>sin mantenerse a flote</i>, o dicho de otro modo, sin estar online.</p
               </body>
              </html>'''
...
class Naufrago:
    ...
    if(data == 'single'):
     ...
     if liststore_font_style == 'bold': # Si antes era bold...
      ...
      if m is not None:
       ...
       feed_label = nombre_feed + ' [' + str(count) + ']'
      font_style = 'bold'
     print '(toggle_leido) feed_label: ' + feed_label
     model.set(iter, 0, feed_label, 3, font_style)
     # END NAME PARSING #
 ...
 def list_row_selection(self, event):
  ...
  (model, iter) = self.treeselection2.get_selected()
  if(iter is not None): # Hay alguna fila de la lista seleccionada
   self.set_statusbar_message(True, 'Visualizando entrada del feed...')
   fecha = self.liststore.get_value(iter, 0)
   ...
   self.webview.load_string(row[1], "text/html", "utf-8", "valid_link")
   ...
   if liststore_font_style == 'bold': # Si la entry era bold, cabe actualizar el nodo del feed
    ...
    if m is not None:
     ...
     model.set(iter, 0, feed_label, 3, font_style)
   # END NAME PARSING #
   self.set_statusbar_message(False, None)
 ...
 def tree_row_selection(self, event):
  ...
  if(iter is not None): # Hay algún nodo seleccionado
   ...
   if(model.iter_depth(iter) == 1): # Si es hoja, presentar entradas
    self.set_statusbar_message(True, 'Generando entradas del feed...')
    print 'Feed *' + row_name + '* seleccionado'
 ...
 def create_main_window(self):
  ...
  self.statusbar_context_id = self.statusbar.get_context_id("Main Statutbar")
  #self.statusbar.push(self.statusbar_context_id, "Texto de estado...")
  # Con un self.statusbar.pop(self.statusbar_context_id) lo quitaríamos.
 ...
 def navigation_requested(self, frame, request, navigation_action, policy_decision, data=None):
  ...
  valid_links = ['valid_link', "file://"+index_path]
  uri = navigation_action.get_uri()
  print "Request to go to %s" % uri
  if uri in valid_links:
   return 0 # Let browse
  else:
 ...
 def hover_link(self, title, uri, data=None):
  ...
  if data is not None:
   self.set_statusbar_message(True, data)
  else:
   self.set_statusbar_message(False, None)
 ...
 def add_feed(self, data=None):
  ...
  if((data is not None) and (type(data) is not gtk.Action)): #  Modo edición I
   ...
   entryName = gtk.Entry(max=38)
   entryName.set_text("  **Se cogerá el título del feed** ")
  ...
  if((textName != '') and (textURL != '') and (response == gtk.RESPONSE_OK)):
   ...
   if((data is not None) and (type(data) is not gtk.Action)): # Modo edición III
    ...
    if(cursor.fetchone() is None):
     ...
     row = cursor.fetchone()
     self.get_favicon(row[0]+1, textURL) # Downloads favicon & adds to the icon factory.
     son = self.treestore.append(iter, [textName, str(row[0]+1), row[0]+1, 'normal'])
...
#   treeview.expand_row(path[:i+1], open_all=False)
 ...
 def get_feed(self, data=None, new=False):
  ...
  self.set_statusbar_message(True, 'Obteniendo feed...')
  new_posts = False # Reset

  if(data is None): # Iterarlo todo, como hace update_feeds_model()
      ...
      for i in range(0, count):
       ...
       if(cursor.fetchone() is None):
        ...
        if((row2 is not None) and (row2[0]>=self.num_entries)):
         ...
         cursor.execute('DELETE FROM articulo WHERE fecha = (SELECT MIN(fecha) FROM articulo WHERE importante = 0 AND id_feed = ?) AND id_feed = ?', [id_feed,id_feed])
         conn.commit()
        ...
        cursor.execute('INSERT INTO articulo VALUES(null, ?, ?, ?, ?, 0, 0, ?)', [title,description,secs,link,id_feed])
        conn.commit()
        new_posts = True
       else:
 ...
 def import_feeds(self, data=None):
  ...
  response = dialog.run()
  if response == gtk.RESPONSE_OK:
   self.set_statusbar_message(True, 'Importando feeds...')
 ...
 def export_feeds(self, data=None):
  ...
  response = dialog.run()
  if response == gtk.RESPONSE_OK:
   self.set_statusbar_message(True, 'Exportando feeds...')
 ...
 def get_favicon(self, id_feed, url):
  ...
  self.set_statusbar_message(True, 'Obteniendo favicon...')
 ...
 def set_statusbar_message(self, put, message=None):
  """Sets & unsets the statusbar messages. Useful for long-term actions."""
  if put == True:
