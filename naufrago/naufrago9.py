...
# Other locale-related stuff
index_path = '/usr/share/naufrago/index.html'
if "es" in locale:
 puf_path = '/usr/share/naufrago/puf_es.html'
elif "ca" in locale:
 puf_path = '/usr/share/naufrago/puf_ca.html'
else:
 puf_path = '/usr/share/naufrago/puf.html'

window_visible = True
current_path = os.getcwd()
db_path = os.getenv("HOME") + '/.naufrago/naufrago.db'
favicon_path = os.getenv("HOME") + '/.naufrago/favicons'
images_path = os.getenv("HOME") + '/.naufrago/imagenes'
ABOUT_PAGE = ''
PUF_PAGE = ''
...
class Naufrago:
 ...
 def help_about(self, action):
  ...
  about.set_logo(gtk.gdk.pixbuf_new_from_file("/usr/share/naufrago/Viking_Longship.svg"))
  about.run()
  about.destroy()
 ...
 def create_ui(self, window):
  ...
  pixbuf = gtk.gdk.pixbuf_new_from_file("/usr/share/naufrago/SRD_RSS_Logo_mini.png")
 ...
 def create_base(self):
  ...
  if not os.path.exists(db_path):
   ...
   self.conn.commit()
   os.makedirs(os.getenv("HOME") + '/.naufrago/favicons')
   os.makedirs(os.getenv("HOME") + '/.naufrago/imagenes')
  else:
   self.conn = sqlite3.connect(db_path, check_same_thread=False)
 ...
 def create_main_window(self):
  ...
  self.window.set_icon_from_file("/usr/share/naufrago/Viking_Longship.svg")
 ...
 def create_trayicon(self):
  ...
  self.statusicon.set_from_file('/usr/share/naufrago/Viking_Longship.svg')
...
#   treeview.expand_row(path[:i+1], open_all=False)
 ...
 def check_feed_item(self, dentry):
  ...
  if(hasattr(dentry,'id')):
   ...
   print 'No tiene id...'
   if description != '':
    print '...se le crea uno por descripción!'
    id = hashlib.md5(description).hexdigest().encode("utf-8")
   else:
    print '...se le crea uno por title!'
    ...
    if(model.iter_depth(iter) == 0): # Si es padre
     ...
     for i in range(model.iter_n_children(iter)):
      ...
      for i in range(0, limit):
       ...
       print 'Comprobando existencia del artículo ('+title+')...'
       cursor.execute('SELECT id FROM articulo WHERE entry_unique_id = ? AND id_feed = ?', [id,id_feed])
       ...
       if(unique is None):
        ...
        if((row2 is not None) and (row2[0]>=self.num_entries)):
         print 'Capacidad del feed sobrepasadao; comprobando frescura del artículo...'
         ...
         if secs > fecha:
          ...
          print 'Articulo fresco; purgando una entrada para hacer espacio...'
          ...
          print 'Artículo no-fresco, skipping...'
        else:
         print 'Salvando entrada en BD...'
         images = self.find_entry_images(feed_link, description)
         cursor.execute('INSERT INTO articulo VALUES(null, ?, ?, ?, ?, 0, 0, ?, ?, ?)', [title.decode("utf-8"),description.decode("utf-8"),secs,link.decode("utf-8"),images,id_feed,id.decode("utf-8")])
        ...
        print 'La entrada existe, skipping...'
        # START Offline mode image retrieving
        if (self.offline_mode == 1):
         cursor.execute('SELECT imagenes FROM articulo WHERE id = ?', [unique[0]]) # ¿Hay imagenes?
         imagenes = cursor.fetchone()
      ...
      if new_posts == True:
       print 'Se encontraron nuevos posts!'
