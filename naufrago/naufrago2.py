...
#############
 ...
 def help_about(self, action):
  ...
  dialog = gtk.MessageDialog(self.window, (gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT), gtk.MESSAGE_INFO, gtk.BUTTONS_OK, "¡No hagan esto en su casa!")
  dialog.run()
  dialog.destroy()
 ...
 def add_category(self, data=None):
  ...
  if((text != '') and (response == gtk.RESPONSE_OK)):
   ...
   conn = sqlite3.connect(db_path)
   cursor = conn.cursor()
   cursor.execute('SELECT id FROM categoria WHERE nombre LIKE ?', [text])
   data = cursor.fetchone()
   if(data is None):
    print 'No category found, inserting...'
    dad = self.treestore.append(None, [text, gtk.STOCK_OPEN, True])
    cursor.execute('INSERT INTO categoria VALUES(null, ?)', [text])
    conn.commit()
   else:
    print 'Category found, skipping!'
   cursor.close()
 ...
 def delete_category(self, data=None):
  ...
  if(iter is not None): # Si hay algún nodo seleccionado...
   #if(len(model.get_path(iter))==1): # ... y es un nodo padre
 ...
 def add_feed(self, data=None):
  ...
  if((textName != '') and (textURL != '') and (response == gtk.RESPONSE_OK)):
   ...
   conn = sqlite3.connect(db_path)
   cursor = conn.cursor()
   cursor.execute('SELECT id FROM feed WHERE nombre LIKE ? OR url LIKE ?', [textName,textURL])
   data = cursor.fetchone()
   if(data is None):
    print 'No feed found, inserting...'
 ...
 def delete_feed(self, data=None):
  ...
  (model, iter) = self.treeselection.get_selected()
  if(iter is not None): # Si hay algún nodo seleccionado...
   #if(len(model.get_path(iter))==2): # ... y es un nodo hijo
